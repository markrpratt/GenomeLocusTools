...
%% All possible 3-mers
nH = 256*H ./ repmat(sum(H),numel(hh),1);
Tmer = cell(256,1);
for i = 1:256
    j = int16(i-1);
    Tmer{i} = cmap([mod(j,4) mod(bitshift(j,-2),4) mod(bitshift(j,-4),4) ...
        bitshift(j,-6)]+1);
end
gH = 256*sum(H,2)/sum(H(:));
...
hold off;
colormap hsv;
set(gca,'xtick',hh+1,'xticklabel',sTmer,'xgrid','on','ygrid','on',...
    'xlim',[1 256],'fontsize',8);
xtickangle(75);
legStr = chrStr;
legStr{end+1} = 'Genome';
